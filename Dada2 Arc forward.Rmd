...
```{r}
path <- "~/Arc" #nom du dossier principal, doit contenir un dossier "fasta" contenant les raw
...
```{r echo=TRUE}
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(300,300),
                     maxN=0, maxEE=2 , truncQ=2, trimLeft=0,
...
```{bash eval=FALSE}
fasttree -nt -gtr < ~/ArcForward/ASV.align.fasta > ~/Arc/tree
```
