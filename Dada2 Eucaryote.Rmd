---
title: "Traitement des séquences en ASV"
...
```{r}
path <- "/home/bgroult/TestEuc" #nom du dossier principal, doit contenir un dossier "fasta" contenant les raw
...
On choisit d'utiliser l'algorithme de DADA2 qui permet de ne pas imposer de seuil (97%) comme pour l'assemblage des OTU, ici on sépare les variant ayant une différence d'un seul nucléotide.
...
seqtab <- seqtab[,nchar(colnames(seqtab)) %in% 240:256]
```
...
On utilise une classification séquentiel impliquant 2 bases de données, la base de donnée **SILVA_SSU_v138** derniére version de SILVA téléchargée sur [DECIPHER](http://www2.decipher.codes/Downloads.html) pour étre sous forme d'objet R. Puis la base de donnée **PR2** mise sous format `RData`, cette base de donnée est spécifique à l'ARNr 18s. 
...
Les rang taxonomiques non identifié sont identifiés comme `NA` 
```{r}
...
Reclassifier avec pr2

```{r}
load("~/TestEuc/pr2_trainingset.1000.RData.gz") # CHANGE TO THE PATH OF YOUR TRAINING SET
...
taxid2<- fill(taxid, names(taxid),.direction = "down")
...
```{r}
taxid <-subset(as.data.frame(taxid), domain =="Eukaryota")
...
write.table(as.data.frame(taxid), file = file.path(path,"ASV.tax.txt"))
...
Le Fasta aligné peut ensuite étre utilisé dans FAST TREE!
...
#Tree.midroot <- phytools::midpoint.root(Tree) # Cette fonction utilise trop de mémoire sur des dataset important
